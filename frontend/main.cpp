...
int inExMod = 31;
u64 inNumCluster = 16;
u64 numberTestA = 1 << 11;
u64 numberTestB = 1 << 11;
u64 numInteration = 20;



void party0_SML_Dist()
{
	Timer timer;
	IOService ios;
	...
	std::vector<std::vector<Word>> SharePoint(p0.mTotalNumPoints);
	for (u64 i = 0; i < p0.mTotalNumPoints; i++)
	{
		SharePoint[i].resize(p0.mDimension);
		for (u64 d = 0; d < p0.mDimension; d++)
		{
			memcpy((u8*)&SharePoint[i][d], (u8*)&p0.mSharePoint[i][d].mArithShare, sizeof(Word));
		}
	}

	timer.setTimePoint("StartDistDone");
